...
//				LogUtils.e("Error occured in updateListener, recording is aborted");
                // stop();
            }
        }
        ...
        try {
            ...
            if (rUncompressed) { // RECORDING_UNCOMPRESSED
                ...
                if (bufferSize < AudioRecord.getMinBufferSize(sampleRate,
                    ...
                    LogUtils.w("Increasing buffer size to "
            ...
            if (e.getMessage() != null) {
                ...
                LogUtils.e("Unknown error occured while initializing recording");
            }
            state = State.ERROR;
        }
    }
    ...
    public void setOutputFile(String argPath) {
        try {
            ...
            if (e.getMessage() != null) {
                ...
                LogUtils.e("Unknown error occured while setting output path");
            }
            state = State.ERROR;
        }
    }
            ...
            if (state == State.INITIALIZING) {
                if (rUncompressed) {
                    if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED)
                        ...
                        LogUtils.e("prepare() method called on uninitialized recorder");
                        state = State.ERROR;
                    }
                } else {
...
//				LogUtils.e("prepare() method called on illegal state");
                release();
                state = State.ERROR;
            }
        } catch (Exception e) {
            if (e.getMessage() != null) {
                ...
                LogUtils.e("Unknown error occured in prepare()");
            }
            state = State.ERROR;
        }
    }
        ...
        if (state == State.RECORDING) {
            ...
            if ((state == State.READY) & (rUncompressed)) {
                try {
                    ...
                    LogUtils.e("I/O exception occured while closing output file");
                }
    ...
    public void start(AudioRecorderListener listener) {
        if (state == State.READY) {
            ...
            LogUtils.e("start() called on illegal state");
            state = State.ERROR;
        }
    }
    ...
    public void stop() {
        if (state == State.RECORDING) {
            ...
            LogUtils.e("stop() called on illegal state");
            state = State.ERROR;
        }
    }
