...
final class SobotInertiaTimerTask extends TimerTask {
    ...
    SobotInertiaTimerTask(SobotWheelView loopview, float velocityY) {
        super();
        loopView = loopview;
    ...
    public final void run() {
        ...
        if (Math.abs(a) >= 0.0F && Math.abs(a) <= 20F) {
            ...
            loopView.handler.sendEmptyMessage(SobotMessageHandler.WHAT_SMOOTH_SCROLL);
            return;
        }
        ...
        loopView.handler.sendEmptyMessage(SobotMessageHandler.WHAT_INVALIDATE_LOOP_VIEW);
    }
