...
//            a.recycle();//回收内存
    ...
    private void initLoopView(Context context) {
        ...
        handler = new SobotMessageHandler(this);
        gestureDetector = new GestureDetector(context, new SobotLoopViewGestureListener(this));
    ...
    void smoothScroll(ACTION action) {//平滑滚动的实现
        ...
        mFuture = mExecutor.scheduleWithFixedDelay(new SobotSmoothScrollTimerTask(this, mOffset), 0, 10, TimeUnit.MILLISECONDS);
    }

    protected final void scrollBy(float velocityY) {//滚动惯性的实现
        cancelFuture();
        mFuture = mExecutor.scheduleWithFixedDelay(new SobotInertiaTimerTask(this, velocityY), 0, VELOCITYFLING, TimeUnit.MILLISECONDS);
    }
    ...
    public final void setOnItemSelectedListener(SobotOnItemSelectedListener OnItemSelectedListener) {
        this.onItemSelectedListener = OnItemSelectedListener;
    }

    public final void setAdapter(SobotWheelAdapter adapter) {
    ...
    private String getContentText(Object item) {
        if (item == null) {
            return "";
        } else if (item instanceof SobotIPickerViewData) {
            return ((SobotIPickerViewData) item).getPickerViewText();
