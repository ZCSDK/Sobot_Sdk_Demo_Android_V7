...
public class SobotSearchRegionAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder vh, int i) {
        ...
        if(tempCode.equals(code)){
            viewHolder.iv_exts.setImageResource(R.drawable.sobot_work_order_selected_mark);
        ...
        if(text.contains(select)){
            spannableString.setSpan(new ForegroundColorSpan(Color.parseColor("#FFFA8314")), text.lastIndexOf(select),text.lastIndexOf(select)+select.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
