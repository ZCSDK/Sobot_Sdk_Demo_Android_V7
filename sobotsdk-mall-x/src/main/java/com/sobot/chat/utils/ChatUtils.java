...
public class ChatUtils {
    ...
    public static void sendPicByUri(Context context,int readFlag, Handler handler,
                                    ...
                                    final SobotMsgAdapter messageAdapter, boolean isCamera, int currentModel) {
        if (initModel == null) {
            return;
        }
        ...
        if (!TextUtils.isEmpty(picturePath)) {
            ...
            if (tmpFile.exists() && tmpFile.isFile()) {
                sendPicLimitBySize(readFlag,picturePath, initModel.getCid(),
                        initModel.getPartnerid(), handler, context, lv_message, messageAdapter, isCamera, currentModel, initModel);
            }
        } else {
            ...
            sendPicLimitBySize(readFlag,file.getAbsolutePath(),
                    initModel.getCid(), initModel.getPartnerid(), handler, context, lv_message, messageAdapter, isCamera, currentModel, initModel);
        }
    }
    ...
    public static void sendPicLimitBySize(int readFlag,String filePath, String cid, String uid,
                                          ...
                                          final SobotMsgAdapter messageAdapter, boolean isCamera, int currentModel, ZhiChiInitModeBase initModel) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            ...
            if (bitmap != null) {
                ...
                if (size < (20 * 1024 * 1024)) {
                    ...
                    sendImageMessageToHandler(readFlag,filePath, handler, id);
                    sendPicture(context,readFlag, cid, uid, filePath, handler, id, lv_message,
     ...
     * @return
     */
    public static String getLogicTitle(Context context, boolean ignoreLogic, String title) {
        if (ignoreLogic) {
            return title;
        } else {
    ...
    /**
     * 根据逻辑获取应该显示的头部公司名称 4.0.0 之后版本新增
     *
     * @param context
     * @param ignoreLogic
     * @param companyName
     * @return
     */
    public static String getLogicCompany(Context context, boolean ignoreLogic, String
            companyName) {
        if (ignoreLogic) {
            return companyName;
        } else {
            int titleDisplayMode = SharedPreferencesUtil.getIntData(context, ZhiChiConstant
                    .SOBOT_CHAT_COMPANY_DISPLAY_MODE, SobotChatCompanyDisplayMode.Default.getValue());
            if (SobotChatCompanyDisplayMode.ShowFixedText.getValue() == titleDisplayMode) {
                //显示固定文本
                String titleContent = SharedPreferencesUtil.getStringData(context, ZhiChiConstant
                        .SOBOT_CHAT_COMPANY_DISPLAY_CONTENT, "");
                if (!TextUtils.isEmpty(titleContent)) {
                    return titleContent;
                } else {
                    //显示公司名称
     ...
     * @return
     */
    public static String getLogicAvatar(Context context, boolean ignoreLogic, String avatar) {
        if (ignoreLogic) {
            return avatar;
        } else {
            int titleDisplayMode = SharedPreferencesUtil.getIntData(context, ZhiChiConstant
                    .SOBOT_CHAT_AVATAR_DISPLAY_MODE, SobotChatAvatarDisplayMode.Default.getValue());
            if (SobotChatAvatarDisplayMode.ShowFixedAvatar.getValue() == titleDisplayMode) {
                //显示固定头像
